...
export class DailyExpDataService {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.dailyExpListRefKey.snapshotChanges()
        .subscribe(actions => {
          ...
          this.allExpListsKeys.length = 0;

          actions.forEach(action => {
            // this.allExpDates.push(action.key)
            this.allExpListsKeys.push(action.key);
            this.allExpLists.push(action.payload.val());

            // console.log(this.allExpListsKeys);
  ...
  add(Exp,Price,date,day) {
    ...
    this.dailyExpList.push(expForm);
  }
  ...
  delete(index){
    let key = this.allExpListsKeys[index];
      this.afDB.list(`UserExpenses/${this.uid}/${this.year}/${this.month}/${key}`).remove();
    }
